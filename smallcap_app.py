...
SEKTOR = {
    # === AI / TEKNOLOGI / DATA CENTER (tren 2026) ===
    "AI_DATACENTER": ["DCII.JK","EDGE.JK","MLPT.JK","MTDL.JK","WIFI.JK","DMMX.JK"],
    "TEKNOLOGI": ["BUKA.JK","EMTK.JK","MSIN.JK","CYBR.JK","WIRG.JK"],
    # === SMALL/MID CAP POPULER & RAMAI 2026 ===
    "ENERGI": ["RAJA.JK","ENRG.JK","INDY.JK","ELSA.JK","DEWA.JK"],
    "TAMBANG": ["DOID.JK","HRUM.JK","BRMS.JK","NCKL.JK","ABMM.JK"],
    "KONSUMER": ["CMRY.JK","ROTI.JK","ULTJ.JK","CLEO.JK","KEJU.JK"],
    "PROPERTI": ["BSDE.JK","DMAS.JK","KIJA.JK","PANI.JK"],
    "INDUSTRI": ["ARNA.JK","MAIN.JK","SRTG.JK","TOTO.JK"],
    "KESEHATAN": ["MIKA.JK","HEAL.JK","PRDA.JK","SILO.JK"],
    "RITEL": ["ERAA.JK","RALS.JK","LPPF.JK","MAPB.JK"],
    "FINANSIAL": ["BFIN.JK","BTPS.JK","PNLF.JK","AMAR.JK"],
}
...
def analyze(symbol, mbonus, market_regime):
    ...
    # === PROTEKSI SMALL CAP 1: FILTER LIKUIDITAS ===
    # Nilai transaksi harian rata-rata (Rp) = harga × volume
    nilai_transaksi = harga * avgv
    likuid_ok = nilai_transaksi >= 1_000_000_000  # min Rp 1 miliar/hari
    # hitung berapa hari volume = 0 (saham sepi/tidur)
    hari_sepi = sum(1 for v in vol[-20:] if v == 0)

    # === PROTEKSI SMALL CAP 2: DETEKSI GORENGAN ===
    goreng_warning = []
    # a) Lonjakan harga ekstrem (naik >25% dalam 1 hari = tanda goreng)
    pc1 = (cl[-1]-cl[-2])/cl[-2]*100 if len(cl)>1 else 0
    if pc1 > 25: goreng_warning.append("🔥 Naik ekstrem >25% sehari (rawan goreng)")
    # b) Volume meledak tak wajar (>10x rata-rata = pompa bandar)
    if vr > 10: goreng_warning.append("🔥 Volume meledak >10x (rawan pompa)")
    # c) Harga sangat murah (gocap-an, rawan dimainkan)
    if harga < 100: goreng_warning.append("⚠️ Harga <Rp100 (rawan dimainkan bandar)")
    # d) Kenaikan beruntun curam (naik >60% dalam 5 hari)
    pc5 = (cl[-1]-cl[-6])/cl[-6]*100 if len(cl)>5 else 0
    if pc5 > 60: goreng_warning.append("🔥 Naik >60% dalam 5 hari (sangat rawan)")
    is_goreng = len(goreng_warning) > 0
    ...
    # tag proteksi small cap
    if is_goreng: tags.append("🔥 RAWAN GORENG")
    if not likuid_ok: tags.append("💧 KURANG LIKUID")

    # Syarat kualitas: ada akumulasi ATAU breakout = jejak "smart money"
    smart_money = is_accum or is_breakout

    # ─── KEPUTUSAN DIPERKETAT (+ PROTEKSI SMALL CAP) ───
    if is_goreng:
        dec="🚫 AVOID"  # saham rawan goreng = JANGAN beli, apapun skornya
    elif not likuid_ok or hari_sepi >= 3:
        dec="🚫 AVOID"  # tidak likuid = bahaya, susah jual
    elif fake or dist: dec="🚫 AVOID"
    ...
    if is_goreng or not likuid_ok or fake or dist or fs<30: risk="HIGH"
    ...
    return {"symbol":symbol.replace(".JK",""),"sector":sector,"harga":harga,
            ...
            "detail":detail,"is_goreng":is_goreng,"goreng_warning":goreng_warning,
            "likuid_ok":likuid_ok,"nilai_transaksi":nilai_transaksi,"hari_sepi":hari_sepi}
...
with c1:
    st.markdown("# 💎 Small Cap & AI Scanner")
    st.caption("Small/Mid Cap Populer + Saham AI/Data Center 2026 · Proteksi Likuiditas + Anti-Gorengan")
    st.caption(f"Swing 1-6 hari · Target 5% · Filter Bandar Aktif · {datetime.now().strftime('%d %b %Y · %H:%M')}")
with c2:
...
st.warning("""⚠️ **PERINGATAN** — Daftar ini berisi small/mid cap populer + saham AI/Data Center 2026
(DCII, EDGE, MLPT dll). Risiko TINGGI: small cap volatil & rawan goreng; saham AI sering bervaluasi
sangat MAHAL/premium (PBV tinggi) jadi rawan koreksi tajam. Scanner sudah menyaring saham tidak likuid
& rawan goreng (tag 🔥 RAWAN GORENG / 💧 KURANG LIKUID → AVOID), tapi tetap WAJIB pakai stop loss &
position size kecil. Saham AI mahal ≠ pasti naik. Hanya pakai uang yang siap hilang.""")
...
with t5:
    ...
    if results:
        ...
        # Peringatan small cap khusus
        if s.get("is_goreng"):
            st.error("🔥 **SAHAM RAWAN GORENG!** " + " · ".join(s.get("goreng_warning",[])) + " — Scanner kasih AVOID. Hindari, ini rawan jebakan bandar.")
        if not s.get("likuid_ok"):
            nt = s.get("nilai_transaksi",0)/1e9
            st.warning(f"💧 **KURANG LIKUID** — Nilai transaksi cuma ~Rp {nt:.2f} miliar/hari (min sehat: Rp 1 M). Susah jual saat butuh.")
        elif s.get("nilai_transaksi"):
            nt = s.get("nilai_transaksi",0)/1e9
            st.caption(f"💧 Likuiditas: ~Rp {nt:.1f} miliar/hari (cukup likuid)")
        st.markdown("---")
    ...
    if st.button("▶️ Jalankan Backtest (±1 menit)"):
        ...
        for i,s in enumerate(WATCHLIST):
            ...
            try:
                ...
                gagal.append(s.replace(".JK","")); continue
        prog2.empty()
        if gagal: st.caption(f"ℹ️ {len(gagal)} saham dilewati (data tidak lengkap)")
        if bt:
